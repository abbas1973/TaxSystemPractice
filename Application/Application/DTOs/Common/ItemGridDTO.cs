...
{
    #region گرفتن اطلاعات گرید
    /// <summary>
    /// گرفتن صفحه بندی جهت نمایش گرید یک موجودیت خاص 
    /// </summary>
    /// <typeparam name="T">نوع موجودیت</typeparam>
    public class ItemGridDTO<T> : IItemListDTO<T> where T : class
    {
        #region Constructors
        public ItemGridDTO() { }

        public ItemGridDTO(int page, int pageLength, int totalCount, int filteredCount, List<T> items)
        {
            Page = page;
